        ...
        <div class="flex justify-between h-16">
                ...
                @if (Auth::user()->role === 'admin')
                    <div class="shrink-0 flex items-center">
                        <a href="{{ route('dashboard') }}" class="flex items-center gap-2 hover:opacity-80 transition">
                            ...
                            <span class="font-bold text-xl text-gray-900">Tambal<span
                                    class="text-blue-600">Finder</span></span>
                            <span
                                class="text-[10px] font-bold bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full border border-gray-200 uppercase tracking-wider">Admin</span>
                        </a>
                    </div>
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                        <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="group flex items-center gap-2">
                            <i
                                class="fa-solid fa-table-columns text-gray-400 group-hover:text-blue-600 {{ request()->routeIs('dashboard') ? 'text-blue-600' : '' }}"></i>
                            {{ __('Halaman Utama') }}
                        </x-nav-link>

                        <x-nav-link :href="route('admin.map')" :active="request()->routeIs('admin.map')" class="group flex items-center gap-2">
                            <i
                                class="fa-solid fa-map text-gray-400 group-hover:text-blue-600 {{ request()->routeIs('admin.map') ? 'text-blue-600' : '' }}"></i>
                            {{ __('Peta') }}
                        </x-nav-link>

                        <x-nav-link :href="route('admin.orders.index')" :active="request()->routeIs('admin.orders.index')" class="group flex items-center gap-2">
                            <i
                                class="fa-solid fa-clipboard-list text-gray-400 group-hover:text-blue-600 {{ request()->routeIs('admin.orders.index') ? 'text-blue-600' : '' }}"></i>
                            {{ __('Pesanan') }}
                        </x-nav-link>

                        <x-nav-link :href="route('admin.stats')" :active="request()->routeIs('admin.stats')" class="group flex items-center gap-2">
                            <i
                                class="fa-solid fa-chart-line text-gray-400 group-hover:text-blue-600 {{ request()->routeIs('admin.stats') ? 'text-blue-600' : '' }}"></i>
                ...
                @elseif(Auth::user()->role === 'owner')
                    <div class="shrink-0 flex items-center">
                        <a href="{{ route('owner.dashboard') }}"
                            ...
                            <span class="font-bold text-xl text-gray-900">Tambal<span
                                    class="text-orange-500">Mitra</span></span>
                        </a>
                    </div>
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                        <x-nav-link :href="route('owner.dashboard')" :active="request()->routeIs('owner.dashboard') || request()->routeIs('owner.order.show')" class="group flex items-center gap-2">
                            <i
                                class="fa-solid fa-shop text-gray-400 group-hover:text-orange-600 {{ request()->routeIs('owner.dashboard') ? 'text-orange-600' : '' }}"></i>
                            {{ __('Dashboard Order') }}
                        </x-nav-link>
                    </div>
                @else
                    <div class="shrink-0 flex items-center">
                        <a href="{{ route('landing') }}" class="flex items-center gap-2 hover:opacity-80 transition">
                            ...
                            <span class="font-bold text-xl text-gray-900">Tambal<span
                                    class="text-green-600">Finder</span></span>
                        </a>
                    </div>
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                        ...
                        <x-nav-link :href="route('landing')" :active="request()->routeIs('landing')" class="group flex items-center gap-2">
                            <i
                                class="fa-solid fa-house text-gray-400 group-hover:text-green-600 {{ request()->routeIs('landing') ? 'text-green-600' : '' }}"></i>
                            {{ __('Home') }}
                        </x-nav-link>

                        <x-nav-link :href="route('peta.index')" :active="request()->routeIs('peta.index') || request()->routeIs('booking.create')" class="group flex items-center gap-2">
                            <i
                                class="fa-solid fa-map-location text-gray-400 group-hover:text-green-600 {{ request()->routeIs('peta.index') || request()->routeIs('booking.create') ? 'text-green-600' : '' }}"></i>
                            {{ __('Cari Bengkel') }}
                        </x-nav-link>

                        <x-nav-link :href="route('booking.history')" :active="request()->routeIs('booking.history') || request()->routeIs('booking.show')" class="group flex items-center gap-2">
                            <i
                                class="fa-solid fa-clock-rotate-left text-gray-400 group-hover:text-green-600 {{ request()->routeIs('booking.history') || request()->routeIs('booking.show') ? 'text-green-600' : '' }}"></i>
                            {{ __('Riwayat') }}
                        </x-nav-link>
                    </div>
                @endif
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                            class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-full text-gray-500 bg-gray-50 hover:text-gray-700 hover:bg-gray-100 focus:outline-none transition ease-in-out duration-150 gap-2">
                            <div
                                class="h-8 w-8 rounded-full flex items-center justify-center font-bold text-white shadow-sm
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden bg-white border-t border-gray-100">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 bg-gray-50">
            <div class="px-4 flex items-center gap-3 mb-3">
                <div
                    class="h-10 w-10 rounded-full {{ Auth::user()->role === 'admin' ? 'bg-blue-600' : (Auth::user()->role === 'owner' ? 'bg-orange-500' : 'bg-green-600') }} flex items-center justify-center text-white font-bold text-lg">
                    {{ substr(Auth::user()->name, 0, 1) }}
                </div>
                <div>
